...
def analyse_trajectory(
    ...
    if len(ordered_samples) < 2 or fps <= 0:
        ...
        return {
            ...
            "leftRightDistance": _unavailable_metric(reason),
            "forwardBackDistance": _unavailable_metric(reason),
            "overallEffort": _unavailable_metric("Effort formula awaiting client validation."),
    ...
    return {
        ...
        "leftRightDistance": {
            "status": "experimental",
            "value": round(sum(abs(v[0]) for v in vectors), 3),
            "unit": distance_unit,
            "note": measurement_note,
        },
        "forwardBackDistance": {
            "status": "experimental",
            "value": round(sum(abs(v[1]) for v in vectors), 3),
            "unit": distance_unit,
            "note": measurement_note,
        },
        "overallEffort": _unavailable_metric("Effort formula awaiting client validation."),
