...
def create_app(
    ...
    async def get_analysis(request: Request, job_id: str) -> dict:
    ...
    async def get_annotated_video(request: Request, job_id: str):
        job = request.app.state.job_manager.get_job(job_id)
        if job is None:
            raise HTTPException(status_code=404, detail="Analysis job was not found.")
    ...
    async def cancel_analysis(request: Request, job_id: str) -> dict:
    ...
    async def delete_analysis(request: Request, job_id: str) -> None:
        try:
